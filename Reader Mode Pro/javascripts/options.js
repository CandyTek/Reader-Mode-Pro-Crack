$(function(){
  ...
  // Get License Info
  chrome.storage.local.get(['cr_license'], function(result) {
    if (result.cr_license) {
      key = result.cr_license.key
      timestamp = result.cr_license.timestamp
      timestamp = Math.floor(timestamp * 1000);
      date = new Date(timestamp);
      date = date.toString().split("GMT")[0];

      $("#cr-license p").html(`<div id='cr-license-key'><b>${key}</b></div> <span class='activated'>Activated on ${date}</span>`);
    } else {
      $("#cr-license").html("You haven't activated the license yet");
    }
  });
